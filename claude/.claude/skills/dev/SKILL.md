...
`references/phase-details.md` の「ユースケース発見手法」セクションに4つの手法（ライフサイクル分析、ステークホルダー×ゴール、ビジネスルール駆動、横断的関心事）の詳細がある。

1. 上記 4 手法を順に適用してシナリオを導出
2. `usecases.md` を生成（正常系 / 異常系 / エッジケース分類、網羅性検証マトリクス付き）
3. **サブエージェントによる網羅性クロスチェック**（ユーザーレビュー前）: `usecases.md` + `prd.md` + Phase 0 のドメイン構造分解を渡し、要件・ビジネスルール・ライフサイクルの漏れを独立チェック。指摘を反映してからレビューに回す
4. 技術判断ログ / ドメインに関する確認事項を記載

このフェーズの成果物は Phase 8（テスト）の入力になる。シナリオがそのままテストケースの元になるため、体系的な網羅性が重要。
...
2. **Storybook プロトタイプを実装**（`general-purpose` サブエージェントに委任）:
   - **Component ストーリー**: 各コンポーネントの全状態（デフォルト、ローディング、エラー、空）
   - **Page ストーリー（MVP レビューの主対象）**: ページ全体の View コンポーネントを分離し、`layout: 'fullscreen'` で全画面状態（Initial / Loading / WithResults / NoResults / Error）をカバー
   - モックデータで動作する状態にする
   - 詳細なパターンとコード例は `references/phase-details.md` の「3階層のストーリー設計」を参照
   - `npm run storybook` でユーザーが確認できるようにする
3. ユーザーに Storybook での動作確認を依頼（Page ストーリーを中心に MVP 観点でレビュー）
...
自動テストが全パスした後、`general-purpose` サブエージェントに MVP-UAT を委任する。
自動テストでは検出しにくい問題（バリデーション不整合、画面遷移の不備、データ不整合等）を早期に発見することが目的。
委任方法の詳細は `references/phase-details.md` の「MVP-UAT 詳細」を参照。
...
- **Phase 6 コンテキストリセット**: Phase 0〜5 の蓄積コンテキストは成果物に集約済みなので、Phase 6 でサブエージェントに md パスを渡してドラフト生成を委任し、実装フェーズに備える

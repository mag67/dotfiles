...
**目的**: ユーザーストーリーを正確に理解し、スコープを確定する。ドメイン知識を徹底的に調査し、**ドメインの構造を分解**して後続フェーズの品質基盤を作る。
...
4. **ドメイン構造の分解**（Phase 2・3 の品質を左右する重要ステップ）:
   ステップ 3 の調査結果を以下の観点で構造化する。「思いつき」ではなく体系的に洗い出すことで、後続フェーズでの要件・ユースケースの漏れを防ぐ。
   - **コアエンティティの特定**: このストーリーに関わる主要な概念・データを列挙する。登場するモノ・ヒト・コトを網羅的に拾う（例: 契約、候補者、企業、契約条件、請求書等）
   - **ライフサイクルの把握**: 各エンティティが「生まれてから終わるまで」にたどる状態遷移を書き出す（例: 契約なら 起案→交渉→締結→履行中→更新/終了/解除）。ライフサイクル上のイベントが Phase 3 のユースケースの入力になる
   - **ステークホルダーと関心事**: 各アクターがこの機能に対して持つ目的・期待・懸念を列挙する。アクターの視点を変えることで見落としがちなシナリオを発見できる
   - **ビジネスルールの抽出**: ドメイン固有の制約条件・計算ルール・判定条件を洗い出す（例: 返金保証期間は入社後90日以内、手数料率は理論年収の35%等）。ルールの漏れは受け入れ基準の漏れに直結する
   - **管理すべきドキュメント・成果物**: 業務プロセスで発生・管理する書類やデータを特定する（例: 契約書原本、覚書、NDA、見積書等）。ドキュメントの作成・保管・検索・閲覧・更新のサイクルを整理する
   この分析結果はユーザーへの確認事項に含め、以降のフェーズの入力として活用する。
5. 以下をユーザーに要約・確認:
   - ユーザーストーリーの理解（「〜として、〜したい。なぜなら〜」形式）
   - **ドメイン構造分解の結果**（エンティティ一覧、ライフサイクル、ビジネスルール等 — 漏れがないかユーザーに確認してもらう）
   - 影響範囲の見込み
...
**目的**: 機能要件・非機能要件を明確にする。Phase 0 のドメイン構造分解を入力として、体系的に要件を導出する。

1. 承認された RFC と **Phase 0 のドメイン構造分解**に基づき `prd.md` を生成:
   - 機能要件（MoSCoW 優先度付き）— 以下の観点で網羅性を確保:
     - **エンティティ起点**: Phase 0 で特定した各エンティティに対して CRUD 操作と状態遷移に必要な機能を洗い出す
     - **ステークホルダー起点**: 各アクターの関心事から必要な機能を導出する（閲覧権限、操作権限、通知要件等）
     - **ビジネスルール起点**: 各ルールの実現に必要な機能を導出する（バリデーション、自動計算、アラート等）
     - **ドキュメント管理**: 管理すべき書類の作成・保管・検索・閲覧・更新・アクセス制御に関する要件
   - 非機能要件（パフォーマンス、セキュリティ、アクセシビリティ等）
   - 受け入れ基準（Given-When-Then 形式推奨）— 各ビジネスルールに対応する基準を必ず含める
   - 制約事項・前提条件
2. **網羅性チェック**: Phase 0 のドメイン構造分解と照合し、抜け漏れがないか確認。特に以下を確認:
   - 各エンティティのライフサイクル上のイベントに対応する要件があるか
   - 各ビジネスルールに対応する受け入れ基準があるか
   - 管理すべきドキュメントの要件が含まれているか
   - 各ステークホルダーの関心事がカバーされているか
3. 技術判断ログ: 非機能要件の数値目標、技術的制約の判断根拠を記録
4. ドメインに関する確認事項: ビジネスルールや優先度で不明な点のみ記載

## Phase 3: ユースケース定義

**目的**: すべてのシナリオを**体系的な発見手法**で網羅的に洗い出す

「思いつくシナリオを列挙する」のではなく、構造的な発見手法でシナリオを導出する。
`references/phase-details.md` の「ユースケース発見手法」セクションに詳細な方法論がある。

1. **ドメインオブジェクト・ライフサイクル分析**: Phase 0 で特定した各エンティティのライフサイクル上のすべてのイベント（作成、更新、状態遷移、削除/終了）に対してシナリオを導出する。ライフサイクルを網羅することで「契約書の保管」「契約の更新」のような見落としがちな操作を捕捉できる
2. **ステークホルダー × ゴールマトリクス**: 各アクターの各ゴールに対して、正常系・異常系のシナリオを導出する。視点を切り替えることで「管理者が契約条件を確認したい」「候補者が契約状況を知りたい」のようなシナリオが浮かび上がる
3. **ビジネスルール駆動シナリオ**: Phase 0 で抽出した各ビジネスルールに対して、ルールが満たされるケースと違反されるケースの両方のシナリオを導出する。ルールごとにシナリオを作ることで、受け入れ基準との整合性を担保する
4. **横断的関心事の掃き出し**: 以下の観点で機能横断的なシナリオを検討する:
   - 権限・認可（誰が何をできるか / できないか）
   - 監査ログ（いつ誰が何をしたかの追跡）
   - 通知（状態変化時のアラート・メール等）
   - ドキュメント管理（原本保管、バージョン管理、検索、アクセス制御）
   - 同時実行・排他制御（複数ユーザーの同時操作）
5. `usecases.md` を生成:
   - 上記 1〜4 で導出したシナリオを正常系 / 異常系 / エッジケースに分類
   - 各シナリオ: アクター、事前条件、トリガー、手順、事後条件、期待結果
   - **網羅性検証マトリクス**: PRD の全要件・全ビジネスルールがユースケースでカバーされていることを確認する表を含める
6. **サブエージェントによる網羅性クロスチェック**（ユーザーレビュー前に実行）:
   `general-purpose` サブエージェントに以下を渡して独立した視点でレビューさせる:
   - `usecases.md`（生成したユースケース）
   - `prd.md`（要件）
   - Phase 0 のドメイン構造分解結果（インテイクの確認事項 or 会話ログ）
   チェック観点:
   - PRD の全 Must Have 要件に対応するユースケースがあるか
   - 全ビジネスルールに正常系・異常系の両方のシナリオがあるか
   - 全エンティティのライフサイクルイベントがカバーされているか
   - ドキュメント管理に関するシナリオが含まれているか（管理すべきドキュメントがある場合）
   サブエージェントの指摘に基づき `usecases.md` を補完してからユーザーレビューに回す。
7. 技術判断ログ: テストカバレッジ方針、シナリオ分類の判断根拠を記録
8. ドメインに関する確認事項: ビジネス上のエッジケースで不明な点のみ記載

このフェーズの成果物は Phase 8（テスト）の入力になる。ここで洗い出したシナリオがそのままテストケースの元になるため、**発見手法に基づく体系的な網羅性**が重要。
...
**目的**: ユースケースに基づくテストで品質を担保し、MVP 観点での受け入れテストを行う
...
自動テストが全パスした後、`general-purpose` サブエージェントに **MVP 観点での受け入れテスト（UAT）** を委任する。
自動テストでは検出しにくい「実際に動かしてみて初めてわかる問題」（バリデーションエラー、画面遷移の不整合、データ不整合等）を早期に発見することが目的。

サブエージェントへの指示:
- `usecases.md` を読み、**Must Have の正常系シナリオ（MVP スコープ）** を対象にする
- 実際にアプリケーションを起動し（`npm run dev` 等）、各シナリオの手順を API 呼び出し or ブラウザ操作で再現する
- 各シナリオの「期待結果」と実際の結果を比較し、差異を報告する
- 発見した不具合を以下の形式で報告:
  - 対象ユースケース ID
  - 再現手順
  - 期待結果 vs 実際の結果
...
4. UAT の結果を確認し、blocker / major を修正
5. 結果のサマリーをユーザーに報告（自動テスト結果 + UAT 結果）
...
|-------|-----------|-------------|----------------|
| 0 | コードベース調査 | `Explore` | 要約結果の受領、ドメイン構造分解、ユーザー確認 |
| 1 | 既存実装の深掘り調査 | `Explore` | rfc.md 生成、レビュー対応 |
| 3 | ユースケース網羅性クロスチェック | `general-purpose` | usecases.md 生成、指摘反映、レビュー対応 |
...
| 8 | テスト種別ごとの作成 + MVP-UAT | `general-purpose` | テスト結果・UAT結果確認、修正指示 |
